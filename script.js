            ...
            const dataObject = JSON.parse(dataJson);
                ...
                const idIconCurrent = dataObject.weather[0].icon;
                    console.log(dataObject);
                        ...
                        span7.innerHTML = "<img src="+"http://openweathermap.org/img/wn/"+idIconCurrent+".png"+">";
                        let dataNow = myData ();
...
function weatherCurrent(data,temperatureCurrent,pressureCurrent,cloudCurrent,windCurrent,humidityCurrent,idIconCurrent) {
                    ...
                    span3.textContent = cloudCurrent;
                        ...
                        span4.textContent = windCurrent;
                            ...
                            span5.textContent = humidityCurrent;
                                let refP7 = document.createElement("p");
                                let p7 = div.appendChild(refP7);
                                p7.innerHTML = "<span></span>";
                                let span7 = p7.lastElementChild;
                                span7.innerHTML = "<img src="+"http://openweathermap.org/img/wn/"+idIconCurrent+".png"+">";
}
    ...
    if(xhr.readyState === 4) {
        if (xhr.status === 200) {    
            ...
            console.log(dataObject);    
                ...
                weatherCurrent(dataNow,temperatureCurrent,pressureCurrent,cloudCurrent,windCurrent,humidityCurrent,idIconCurrent)
...
function weatherThreeHours(dataCurrent,temperatureCurrent,pressureCurrent,cloudCurrent,windCurrent,humidityCurrent,idIconCurrent) {
    const refDivContainer = document.getElementById("container");
    const refDiv = document.createElement("div");
    const div = refDivContainer.appendChild(refDiv);
    div.style.width = "35%";
        ...
        if(fiveDays.readyState === 4) {
            if (fiveDays.status === 200) {
                ...
                for(let i=0; i<dataObject.list.length; i++) {
                    ...
                    const idIconCurrent2 = dataObject.list[0].weather[0].icon;
                    const idIconCurrent = dataObject.list[i].weather[0].icon;
                    let span6 = document.getElementById("span-6");
                    ...
                    iconCurrent(idIconCurrent2);
                   
                    weatherThreeHours(dataCurrent,temperatureCurrent,pressureCurrent,cloudCurrent,windCurrent,humidityCurrent,idIconCurrent)     
                }
            } else {
